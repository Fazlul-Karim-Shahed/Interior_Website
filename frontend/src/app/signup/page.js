...
export default function AdminRegistration() {
    ...
    const [fetching, setFtching] = useState(false);
    ...
    useEffect(() => {
        ...
        getSettingsApi().then((data) => {
            setFtching(false);
            if (!data.error) {
                setVisibility(data.data.signupVisibility);
            }
        });
    }, []);
    ...
    if (fetching === true || !visibility) {
        return (
            <div className="h-screen w-full flex items-center justify-center bg-gradient-to-br from-white via-slate-100 to-gray-50-">
                <div className="bg-white/60 backdrop-blur-lg border border-white/30 shadow-2xl rounded-3xl px-6 py-12 max-w-md text-center transition duration-300 hover:shadow-[0_0_40px_rgba(0,0,0,0.05)]">
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-3">Registration Closed</h2>
                    <p className="text-gray-600 text-sm mb-8">We're sorry, but the registration period for this event has ended. Please check back later or follow us for updates.</p>

                    <Link href="/" className="inline-block bg-red-600 hover:bg-red-700 text-white text-sm font-medium px-6 py-3 rounded-full shadow-md transition duration-200">
                        Go to Homepage
                    </Link>
                </div>
            </div>
